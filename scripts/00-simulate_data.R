...
# Simulating establishment status (e.g., Pass, Conditional Pass, Closed)
establishment_status <- c("Pass", "Conditional Pass", "Closed")
...
simulated_data <- 
  tibble(
    ...
    establishment_status = sample(establishment_status, number_of_records, replace = TRUE)
  )
